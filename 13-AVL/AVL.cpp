...
NO* girarDireita(NO* no)
{
	...
	NO* aux = novoDireito->dir;

	novoDireito->dir = no;
	no->esq = aux;

	no->altura = max(alturaNo(no->dir), alturaNo(no->esq)) + 1;
	novoDireito->altura = no->altura = max(alturaNo(no->dir), alturaNo(no->esq)) + 1;
	return novoDireito;
}

NO* girarEsquerda(NO* no)
{	
	NO* novoEsquerdo = no->dir;
	NO* aux = novoEsquerdo->esq;

	novoEsquerdo->esq = no;
	no->dir = aux;

	no->altura = max(alturaNo(no->esq), alturaNo(no->dir)) + 1;
	novoEsquerdo->altura = no->altura = max(alturaNo(no->esq), alturaNo(no->dir)) + 1;
	return novoEsquerdo;
